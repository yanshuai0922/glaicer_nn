import os

os.environ["CUDA_DEVICE_ORDER"] = "PCI_BUS_ID"
os.environ["CUDA_VISIBLE_DEVICES"] = "5"

# import the necessary packages
from keras.preprocessing.image import img_to_array
from keras.models import load_model
import numpy as np
import argparse
#import imutils
import cv2
import copy
import scipy.io as sio
norm_size = 11
filepath = "/home/ys/ys0922/research_python2/CNN_Keras/Sample/"


def args_parse():
    # construct the argument parse and parse the arguments
    ap = argparse.ArgumentParser()
    ap.add_argument("-m", "--model",
        help="path to trained model model", default="plot_min.h5")
    ap.add_argument("-i", "--image",
                    help="path to input image", default=filepath + 'val/00001/5565.mat')
    ap.add_argument("-s", "--show", action="store_true",
                    help="show predict image", default=False)
    args = vars(ap.parse_args())
    return args


def predict(args):
    # load the trained convolutional neural network
    print("[INFO] loading network...")
    model = load_model(args["model"])

    # load the image
    img = sio.loadmat(args["image"])
    Image = img['SmallImage']
    crop = np.array(Image, dtype="float")
    #print("this is image", crop)
    orig = copy.copy(crop)
    # orig = image.copy()

    # pre-process the image for classification
    if img_i == 1:
        image = np.transpose(crop[None], (1, 2, 0))
    else:
        image = crop
    image = np.expand_dims(image, axis=0)

    # classify the input image
    result = model.predict(image)[0]
    # print (result.shape)
    proba = np.max(result)
    label = str(np.where(result == proba)[0])
    label = "{}: {:.2f}%".format(label, proba * 100)
    print(label)

    if args['show']:
        # draw the label on the image
        output = imutils.resize(orig, width=400)
        cv2.putText(output, label, (10, 25), cv2.FONT_HERSHEY_SIMPLEX,
                    0.7, (0, 255, 0), 2)
        # show the output image
        cv2.imshow("Output", output)
        cv2.waitKey(0)


# python predict.py --model traffic_sign.model -i 2.png -s
if __name__ == '__main__':
    img_i = input("please input number if you input 1 mean sar else mean msi:")
    args = args_parse()
    predict(args)